...
class WriteOnceLuceneIndex( val directory : Directory, val computeTotalHits : Boolean ) extends LuceneIndex {

    private val LOG : Logger = LoggerFactory.getLogger( getClass )

    override def write( block : util.Collection[ Document ] ) : Unit = ???
