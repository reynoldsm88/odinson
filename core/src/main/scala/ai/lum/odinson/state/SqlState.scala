...
object SqlState {
    ...
    def apply( config : Config, luceneIndexOpt : Option[ LuceneIndex ] ) : SqlState = {
        ...
        val dataGathererOpt = luceneIndexOpt match {
            case Some( index ) => Some( DataGatherer( displayField, index ) )
            case None => None
        }
