...
class DataGatherer( val displayField : String, val luceneIndex : LuceneIndex ) {

    val storedFields : Seq[ String ] = luceneIndex.settings.storedFields
    ...
    def getTokens( docID : Int, fieldName : String ) : Array[ String ] = {
        TokenStreamUtils.getTokensFromMultipleFields( docID, Set( fieldName ), luceneIndex )( fieldName )
    }

    def getTokens( docID : Int, fieldNames : Set[ String ] ) : Map[ String, Array[ String ] ] = {
        TokenStreamUtils.getTokensFromMultipleFields( docID, fieldNames, luceneIndex )
    }
...
object DataGatherer {
    ...
    def apply( displayField : String, luceneIndex : LuceneIndex ) : DataGatherer = {
        new DataGatherer( displayField, luceneIndex )
    }
